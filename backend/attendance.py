...
def col_add():
    ...
    command = f'alter table VIT1 add {date1} char(4);'
    try:
...
def take_attendace():
    ...
    class_names = []
    try:
        with open('VIT1_encode.dat', 'rb') as f:
            try:
                while True:
                    data = pickle.load(f)
        ...
        for encodeface, faceloc in zip(encodes_current_frame, faces_cur_frame):
            ...
            if matches[matchindex]:
                ...
                if name not in done_names:
                    if datetime.datetime.now().time() < datetime.time(19,45):
                        state = 'P'
                    else:
                        state = 'AP'  # -> absent-present

                    cursor.execute(f"update VIT1 set {date} ='{state}' where name = '{name}';")
    ...
    cursor.execute(f"update VIT1 set {date} ='A' where {date} is null;")
    cap.release()
...
def take_photo_enocode():
    ...
    adno = input('enter register number')
    ...
    while True:
        ...
        if k % 256 == 27:  # ->escape is pressed
            ...
            file = open('VIT1_encode.dat', 'ab')
            data = [adno, encode, name_, img]
            try:
                pickle.dump(data, file)
                cursor.execute(f"insert into VIT1(name,reg_no) values('{name_}','{adno}');")
